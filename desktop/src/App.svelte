<script lang="ts">
  ...
  import { LOCALE_INFO, setLocale, type SupportedLocale } from "./lib/i18n";
  ...
  let licenseDialog: HTMLDialogElement | null = $state(null);
  ...
  // Sync tray icon con estado actual. Reactivo: cualquier cambio de pttActive/transcribing/delivering
  // dispara set_tray_state automáticamente. Silently catch errors (Tauri puede no estar listo).
  $effect(() => {
    const state = pttActive
      ? "recording"
      : (transcribing || delivering)
        ? "transcribing"
        : "idle";
    invoke("set_tray_state", { state }).catch(() => {});
  });
  ...
  function formatExpiresDate(ts: number): string {
    // Locale fecha lo gestiona el navegador con el idioma del SO (no del UI selector — coherente con Date nativo).
  ...
  async function deliverInner(audioPath: string) {
    ...
    // B2 (2026-05-27): transcribe LOCAL embebido via whisper-rs (whisper.cpp).
    // YA NO depende del oido-daemon python externo en localhost:4331.
    // El modelo se descarga automáticamente la primera vez a ~/.local/share/miloro/models/
    // Settings legacy (whisperBin, modelPath) quedan obsoletos pero no se borran para retrocompatibilidad alpha.
    const raw = await invoke<string>("transcribe_local", {
      audioPath,
      model: settings.whisperModel,        // 'small' por default, hasta 'large-v3' Pro
      language: settings.whisperLanguage,  // 'es', 'en', '' (auto)
    });
    transcribing = false;
    ...
    if (clean && recordingDurationSec > 0) {
      if (isOnlineLicense && licenseKey) {
        reportUsage(licenseKey, recordingDurationSec).then((result) => {
          if (result !== null) {
            licenseInfo = {
              ...licenseInfo,
              seconds_used_today: result.secondsUsedToday,
              quota_exceeded: result.quotaExceeded,
            };
            // Si tras este report acabamos de cruzar el límite, avisar al usuario
            if (result.quotaExceeded && licenseInfo.plan === "free") {
              showToast("🛑 Has alcanzado el límite Free de 30 min/día. Vuelve mañana o upgrade a Pro.", false);
            }
          }
        });
      } else {
...
<main class="app-window">
  <div class="content">
    <div class="brand">
      ...
      <div class="tagline">{$_("app.tagline")}</div>
    </div>
    ...
    <div class="account-bar">
      ...
      {#if isOnlineLicense}
        ...
        <span>·</span>
        <button class="link-btn" onclick={toggleDevicesPanel}>📱 {licenseInfo.devices_used}/{licenseInfo.devices_max} {$_("account.devices_short")}</button>
      {/if}
      ...
      {:else}
        <span>·</span>
        <span>⏳ {licenseInfo.expires_at ? formatExpiresDate(licenseInfo.expires_at) : $_("account.expires_indef")}</span>
      {/if}
      {#if isOnlineLicense}
        <button class="link-btn" onclick={changeLicense}>{$_("header.change_license")}</button>
      {:else}
        <button class="link-btn" onclick={openLicenseDialog}>
          {$_("header.have_license")}
        </button>
      {/if}
    </div>
    ...
    <!-- License como modal HTML5 nativo. NO desplaza componentes del layout. -->
    <dialog
      bind:this={licenseDialog}
      class="license-modal"
      onclose={() => (showLicenseInput = false)}
    >
      <div class="license-modal-content">
        <div class="license-modal-header">
          <h3>{$_("license_modal.title")}</h3>
          <button class="modal-close" onclick={closeLicenseDialog} aria-label={$_("devices.close")}>✕</button>
        </div>
        <p class="hint">{$_("license_modal.hint")}</p>
        <input
          bind:value={licenseKey}
          placeholder={$_("license_modal.placeholder")}
          autocomplete="off"
        />
        <div class="license-modal-actions">
          <button class="btn btn-ghost" onclick={closeLicenseDialog}>{$_("license_modal.cancel")}</button>
          <button class="btn btn-primary" onclick={async () => { await verify(); if (isOnlineLicense) closeLicenseDialog(); }} disabled={!licenseKey}>{$_("license_modal.verify")}</button>
        </div>
        {#if statusMsg}<p class="status-line">{statusMsg}</p>{/if}
        <p class="modal-foot-link">
          {$_("license_modal.no_license_link")} <a href="https://miloro.app/get-started" target="_blank" rel="noopener">{$_("license_modal.get_free")}</a>
        </p>
      </div>
    </dialog>
    ...
    {/if}
        ...
        >
          ...
          <span class="label">
            ...
            {:else if transcribing}
              {lastTranscription || $_("status.transcribing")}
            {:else if delivering}
              {$_("status.delivering")}
            {:else}
              {$_("status.ready")}
      ...
      <div class="cols">
        ...
        <div class="col">
          ...
          <div class="section">
            <div class="section-label">{$_("model.section")}</div>

            <div class="field">
              <span class="field-label">{$_("model.quality")}</span>
              <CustomSelect options={modelOptions} bind:value={settings.whisperModel} />
            </div>

            <div class="field">
              <span class="field-label">{$_("model.language")}</span>
              <CustomSelect options={languageOptions} bind:value={settings.whisperLanguage} />
            </div>

            <div class="field">
              <span class="field-label">{$_("model.output")}</span>
              <CustomSelect options={taskOptions} bind:value={settings.whisperTask} />
            </div>

            <div class="field">
              <span class="field-label">{$_("model.vad")}</span>
              <label class="toggle">
                <input type="checkbox" bind:checked={settings.whisperVad} />
                <span class="slider"></span>
              </label>
            </div>
            <div class="hint-inline">{@html $_("model.vad_hint")}</div>

            <div class="hint-inline">{$_("model.first_download_hint")}</div>
          </div>

          <!-- Sistema (movido aquí desde col 3 vieja — Marc 2026-05-26: layout 2 cols) -->
          <div class="section">
            <div class="section-label">{$_("system.section")}</div>

            <div class="field field-toggle">
              <label class="toggle-row">
                <input type="checkbox" bind:checked={settings.autoLaunch} />
                <span class="toggle-label">{$_("system.autolaunch")}</span>
              </label>
              <p class="hint">{$_("system.autolaunch_hint")}</p>
            </div>

            <div class="field field-toggle">
              <label class="toggle-row">
                <input type="checkbox" bind:checked={settings.autoUpdate} />
                <span class="toggle-label">{$_("system.autoupdate")}</span>
              </label>
              <p class="hint">{$_("system.autoupdate_hint")}</p>
            </div>

            <div class="field">
              <button class="mini-btn" onclick={() => checkForUpdates(true)} disabled={updateState === "checking" || updateState === "downloading"}>
                {#if updateState === "checking"}{$_("system.checking")}
                {:else if updateState === "downloading"}{$_("system.downloading")}
                {:else}{$_("system.check_updates")}{/if}
              </button>
              {#if updateLastMsg}
            ...
            <div class="field field-toggle lang-selector">
              <span class="toggle-label">🌐 {$_("header.language")}</span>
              <div class="lang-buttons">
                {#each LOCALE_INFO as info}
                  <button
                    type="button"
                    class="lang-btn"
                    class:lang-btn-active={$locale?.startsWith(info.code)}
        ...
        <div class="col">
          ...
          <div class="section">
            <div class="section-label">{$_("behavior.section")}</div>

            <div class="field">
              <span class="field-label">{$_("behavior.ptt_key")}</span>
              <CustomSelect options={pttKeyOptions} bind:value={settings.pttKey} />
            </div>
            <div class="hint-inline">{$_("behavior.ptt_hint")}</div>

            <div class="field">
              <span class="field-label">{$_("behavior.clipboard")}</span>
              <label class="toggle">
            ...
            <div class="field">
              <span class="field-label">{$_("behavior.autotype")}</span>
              <label class="toggle">
            ...
            <div class="field">
              <span class="field-label">{$_("behavior.typewriter")}</span>
              <label class="toggle">
            ...
            <div class="field">
              <span class="field-label">{$_("behavior.show_status_tags")}</span>
              <label class="toggle">
                <input type="checkbox" bind:checked={settings.showStatusTags} />
                <span class="slider"></span>
              </label>
            </div>
            <div class="hint-inline">{@html $_("behavior.status_tags_hint")}</div>
          </div>

          <!-- Vocabulario + Texto automático dentro de Col 2 — layout 2 cols (Marc 2026-05-26) -->
          <div class="section">
            <div class="section-label">{$_("vocabulary.section")}</div>
            <div class="field field-textarea">
              <textarea
                bind:value={settings.whisperInitialPrompt}
                placeholder={$_("vocabulary.placeholder")}
              ></textarea>
            </div>
            <div class="hint-inline">{@html $_("vocabulary.hint")}</div>
          </div>

          <div class="section">
            <div class="section-label">{$_("autotext.section")}</div>
            <div class="field field-textarea">
              <label class="field-label-block" for="auto-prefix-input">{$_("autotext.prefix_label")}</label>
              <input
                id="auto-prefix-input"
                type="text"
                bind:value={settings.autoPrefix}
                placeholder={$_("autotext.prefix_placeholder")}
              />
            </div>
            <div class="field field-textarea">
              <label class="field-label-block" for="auto-suffix-input">{$_("autotext.suffix_label")}</label>
              <input
                id="auto-suffix-input"
                type="text"
                bind:value={settings.autoSuffix}
                placeholder={$_("autotext.suffix_placeholder")}
              />
            </div>
            <div class="hint-inline">{$_("autotext.hint")}</div>
          </div>

        </div>
      </div>

      <!-- Última transcripción full-width DEBAJO de las cols + botón Probar grabación -->
      <div class="section full-width-section">
        <div class="section-label">
          <span>{$_("test_recording.section")}</span>
        ...
        >
          {lastTranscription || $_("test_recording.empty_hint")}
      ...
      <div class="actions">
        <button class="btn btn-secondary" onclick={testRecord} disabled={recording}>
          {recording ? $_("test_recording.recording") : $_("test_recording.test")}
        </button>
      </div>
      <div class="autosave-hint">{$_("test_recording.autosave_hint")}</div>
...
<style>
  ...
  /* .titlebar dots + title removidos — no se usan (no hay titlebar custom en la app, usamos Tauri default) */
  ...
  .brand .logo:hover {
    ...
    filter: drop-shadow(0 8px 24px rgba(22, 163, 74, 0.45));
    animation: parrot-wiggle 0.6s ease-in-out;
  }
  @keyframes parrot-wiggle {
    0%, 100% { transform: scale(1.15) rotate(-8deg); }
    25% { transform: scale(1.18) rotate(-12deg); }
    50% { transform: scale(1.20) rotate(0deg); }
  ...
  /* === License modal (dialog HTML5 nativo) === */
  /* Reset native dialog defaults para integrar con paleta tropical. */
  .license-modal {
    border: none;
    padding: 0;
  ...
  .license-modal::backdrop {
    ...
    /* backdrop-filter: blur removido — crasheaba WebKitGTK en algunos drivers GPU */
  }
  ...
  .modal-close:hover { background: #FDE68A; }
  .license-modal input {
    width: 100%;
    box-sizing: border-box;
    padding: 0.6rem;
  ...
  .modal-foot-link a { color: #16A34A; text-decoration: none; }
  .modal-foot-link a:hover { text-decoration: underline; }
  ...
  /* Layout 2 cols (Marc 2026-05-26): izq = Modelo + Sistema, der = Comportamiento + Vocabulario + Texto auto */
  ...
  /* :global() necesario porque los <strong>/<code> vienen de {@html $_(...)} (i18n hints)
     y Svelte scope-CSS no los matchea por defecto. */
  :global(.hint-inline strong) { color: #1F2937; }
  :global(.hint-inline code) { background: #FDE68A; padding: 0 4px; border-radius: 3px; font-size: 0.85em; }
  ...
  /* Selector de idioma — fila de banderas seleccionables */
  .lang-selector {
    flex-direction: column;
    align-items: stretch;
    gap: 0.5rem;
  ...
  .lang-btn:hover { background: #FFFBEB; transform: scale(1.08); }
  .lang-btn-active {
    border-color: #16A34A;
    background: #DCFCE7;
