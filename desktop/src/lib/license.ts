...
// Origin sin /api — los handlers añaden /api/... abajo (evita doble /api/ que da 404).
const DEFAULT_BACKEND = "https://miloro.app";

function backendUrl(): string {
  // Override vía localStorage si el Worker está en otro host (ej. workers.dev mientras se configura ruta).
  // IMPORTANTE: el override DEBE ser solo el origin (sin /api), igual que el default.
...
export interface LicenseInfo {
  ...
  /** True si Free tier ha alcanzado/superado 30min/día. Backend ground truth. */
...
/** Resultado de reportUsage — info completa quota del backend. */
export interface UsageReportResult {
  secondsUsedToday: number;
  quotaExceeded: boolean;
  quotaRemainingSeconds: number;  // -1 si unlimited (Pro)
}

/**
 * Reporta segundos de audio transcrito al backend. Non-blocking — si falla, app continúa.
 * Devuelve el estado de quota tras el incremento para que el cliente decida si bloquear futuras.
 * Backend devuelve 429 si quota YA excedida ANTES de este report → tratamos como exceeded inmediato.
 */
export async function reportUsage(key: string, seconds: number): Promise<UsageReportResult | null> {
  ...
  try {
    ...
    if (r.status === 429) {
      // Backend dice quota exceeded antes de este report (Free al límite)
      return {
        secondsUsedToday: typeof data.seconds_used_today === "number" ? data.seconds_used_today : FREE_QUOTA_SECONDS_PER_DAY,
        quotaExceeded: true,
    ...
    if (!r.ok) return null;
    return {
      secondsUsedToday: typeof data.seconds_used_today === "number" ? data.seconds_used_today : 0,
      quotaExceeded: data.quota_exceeded === true,
      quotaRemainingSeconds: typeof data.quota_remaining_seconds === "number" ? data.quota_remaining_seconds : -1,
    };
  } catch {
    return null;
  }
